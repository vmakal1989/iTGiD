...
function f7() {
    ...
    a7.hasOwnProperty(key) ? out7.innerHTML = 1 : out7.innerHTML = 0;
}
...
function f8() {
    ...
    a8.hasOwnProperty(key) ? out8.innerHTML = a8[key] : out8.innerHTML = 0;
}
...
function f10(arr, val) {
    let out;
    for(let key in arr) {
        arr[key] === val ?  out = true :  out = false;
        if(out) break;
    }
    return out;
}
...
function f19() {
    let station = document.querySelector('.i-19').value;
    if(station) station = station[0].toUpperCase() + station.slice(1);
    let out = '';
    for (let key in a19) {
        for(let i = 0; i < a19[key].length; i++) {
            if(a19[key][i] === station) out+= key;
        }
    }
